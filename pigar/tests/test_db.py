...
class DBTests(unittest.TestCase):
    ...
    def test_db(self):
        ...
        def _assert_moduels(modules, deleted=None):
            if deleted is None:
                deleted = set()
        ...
        self._db.store_distribution_with_top_level_modules('pigar', '1.0.0', [])
        row = self._db.query_distribution_by_name('pigar')
        self.assertIsNotNone(row)
        self.assertDictEqual(
            dataclasses.asdict(row), {'name': 'pigar', 'version': '1.0.0'}
        )
